...
# 2. 自动获取最新版本号并下载
# 获取最新的 Tag 名字，例如 v0.0.12
NEW_VER=$(curl -s https://api.github.com/repos/anytls/anytls-go/releases/latest | grep | grep "tag_name" | cut -d : -f 2,3 | tr -d '", ')
# 去掉 v，得到 0.0.12 用于匹配文件名
PURE_VER=${NEW_VER#v}

ARCH=$(uname -m)
if [ "$ARCH" = "x86_64" ]; then
    # 拼接链接: anytls_0.0.12_linux_amd64.zip
...
if [ $? -ne 0 ]; then
    echo -e "${RED}下载失败，请检查网络或版本号是否正确。${NC}"
    exit 1
fi
